...
export default function TituloIndex(props:TProps) {
  ...
  async function deleteRow(id:string){
    if (url)
      await axios.delete(url.concat('/').concat(id))
      .then((response:any) => { 
                    ...
                    },
                    {
                      ...
                      formatter: (cellContent:any, row:any) => (                        
                          <Link to={'/editTitulo/edit/' + row.id_titulo} title='ALTERAR' >
                            <Icon icon="dashicons:edit-large"  color="orange" width="25" height="25" />
                          </Link>                                                                            
                      )
                    },                                                              
                    {
                      ...
                      text: 'Excluir',
                      formatter: (cellContent:any, row:any) => (                        
                          <div style={{display:'flex', justifyContent:'center', alignItems:'center'}} title='EXCLUIR'>
                            <Icon icon="fluent:delete-20-filled" color="red" width="25" height="25" inline={true} onClick={() => { const excluir = window.confirm('Excluir?'); if(excluir){ deleteRow(String(row.id_titulo)) }  }   }/>
                          </div>                       
                      )
                    },                                                      
                  ]
  ...
  if (loading) 
    ...
    return <>  
             ...
             <SForm>
               ...
               <div style={{display:'flex', justifyContent:'center', alignItems:'center'}} title='NOVO'>
                 <Link to={'/editTitulo/new'} >                                  
                   <Icon icon="fluent:add-circle-24-filled" width="50" height="50" color='green' />
                 </Link> 
               </div>                  
             </SForm>                       
           </>   
