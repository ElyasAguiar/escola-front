...
export default function TituloEdit(props:TProps) {
  ...
  const url = 'https://pcn662vet2.execute-api.us-east-1.amazonaws.com/dev/titulo/'
  ...
  async function handleClick() {
    ...
    let titulo: TTitulo = {    // Para edição ainda precisa acrescentar o id
      id_titulo: undefined,
    ...
    if (action === 'new') {
      ...
      await axios.post(url, titulo)
      .then((res:InsertResponse) => {  
          setError('')
          if (res.data.inserted)                
            alert('Título inserido!')
          else
            alert('Título não inserido!')
      })
      .catch((error:any) => {                      
      ...
      await axios.put(url, titulo)
      .then((res:UpdateResponse) => {  
          setError('')
          if (res.data.updated)                
            alert('Título Alterado!')
          else
            alert('Título não Alterado!')
      })
      .catch((error:any) => {            
          setError( Message( error.request.response ) )
      }) 
  ...
  if ( loading ) 
    ...
    return <>  
             <GlobalStyle/>  
             <SForm>
               <Link to={'/'} title='FECHAR (ESC)' style={{display:'flex', justifyContent:'right', alignItems:'right'}} >
                 <Icon icon='ant-design:close-square-filled' color='red' width='50' height='50'/>
               </Link>                                                                            
                                                 
               <STitle> { action === 'new' ? 'Inserindo' : 'Alterando' } Título </STitle>
               ...
               <div style={{display:'flex', justifyContent:'center', alignItems:'center'}} title='SALVAR'>
                   <Icon icon='flat-ui:save' color='green' width='50' height='50' type='submit' onClick={ () => {handleClick()} } />
               </div>                  
